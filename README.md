...
方位角は 12 分割、仰角は 3 段の合わせて 36 セル。同心円の 3 本は外側から

| 帯 | 仰角 | 意味 |
|---|---|---|
| 外周 `下` | −20 度未満 | 被写体を**見上げた**位置から撮った |
| 中央 `横` | −20〜20 度 | ほぼ**水平**から撮った |
| 内側 `上` | 20 度以上 | 被写体を**見下ろした**位置から撮った |

**帯は水平を中心に対称に切ること。** 最初は「水平以下 / 15〜40 度 / 40 度以上」に
していたが、これだと一番外の帯が 105 度ぶんを飲み込む。立って手持ちで撮ると仰角は
ほぼそこに収まるので、**外周しか埋まらず上下の区別が付かなかった**。
境界の 20 度は、1 m 先の被写体に対してカメラを 36 cm 上下させた角度。
意識して動かせば届き、普通に歩き回るだけでは越えない幅として選んである。
...
リングの上端はステータスバーの**実測の下端**に合わせる。XML で余白を決め打つと、
システムバーのインセットぶんステータスバーが伸びたときに重なる。
インセットは端末ごとに違うので、定数では合わせられない。
...
| 仰角の帯は水平を中心に切る（片側に寄せると 1 本しか埋まらない） | 上記「撮った画角のガイド」。`ViewCoverage.BAND_LOWER_DEG` |
| リングの上端は実測のステータスバー高に追従させる（決め打つと重なる） | `MainActivity.applyWindowInsets()` |
...
- **深度の保存は実機で確認済み**（Pixel 8 / ARCore 1.56）。
  ...
  - 残差 52 mm は信頼度の足切りをいくら上げても 49 mm までしか下がらない。
    ...
    そちらを疑う
- **撮った画角のガイドは実機で 1 度動かし、2 点直した。**
  仰角の帯が片側に寄っていて外周しか埋まらなかったのと、リングがステータスバーに
  重なっていた。どちらも直したが、**直したあとは実機未検証**。
  計算は実機データ（上の 4 フレーム）で再現して確かめてあり、帯を切り直すと
  同じ撮影が「水平 2 枚 / 見下ろし 2 枚 / 見上げ 0 枚」に分かれて、
  下から撮っていないことが読めるようになる。
  次の撮影で確認すること:
  - リングがステータスバーに重ならないか、`使い方`／`ガイド`を隠さないか
  - `下` `横` `上` の 3 本が撮り方に応じて別々に埋まるか
